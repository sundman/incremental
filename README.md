...
- **Lab** (science): Scholars produce Research, which you spend on a tech tree. Scholars are bought with Realm Food (1,000 for the first, 50% more for each after). Scholars and Lab Assistants each eat 0.2 Realm Food per second but take no Realm housing or jobs, so a hungry Realm stalls the Lab. Too much science is bad for magic: each Laboratory makes all of Arcana 3% weaker. Techs unlock science buildings in the Realm.
