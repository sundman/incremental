...
- **Lab** (science): Scholars produce Research, which never piles up: you pick one tech to research and Research streams straight into it until it is done, with no build time. Switching to another tech keeps the progress made so far, and a tech's other costs (Planks, Glass and so on) are paid once, when you first pick it. Repeatable techs that cost only Research, like Cartography, keep going level after level. With nothing picked, Research is lost. Buildings stay hidden until everything they require is unlocked. Buildings that use up resources every second (Sawmills, Kilns, Condensers and so on) have an on/off switch, and their upkeep is shown in yellow. The Lab only lists research you can start now; the "Show full research tree" button opens the whole tree, with what is researched, available and locked. Scholars are bought with Realm Food (1,000 for the first, 50% more for each after), and each one needs an idle Realm person, who leaves the Realm for the Lab. Lab Assistants are hired with Realm Gold. Scholars and Lab Assistants each eat 0.2 Realm Food per second but take no Realm housing or jobs, so a hungry Realm stalls the Lab. Too much science is bad for magic: each Laboratory makes all of Arcana 3% weaker. Techs unlock science buildings in the Realm.

Build a **Library** to open the Lab. It's built from Planks and Bricks, so it needs a Sawmill and a Kiln first. Research **Occultism** in the Lab (10,000 Research, so it usually takes a few resets) and the Realm can build a **Shrine** (opens Arcana).
